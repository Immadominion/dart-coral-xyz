...
class EventDefinition {
  ...
  factory EventDefinition.fromIdl(
    IdlEvent idlEvent, {
    ...
    final fields = idlEvent.fields
        .map(
          (field) => EventFieldDefinition.fromIdlField(
            field,
    ...
    final validationRules =
        EventValidationRules.fromFields(fields, config: config);
  ...
  bool isCompatibleWith(EventDefinition other) {
    ...
    for (final field in fields) {
      final otherField =
          other.fields.where((f) => f.name == field.name).firstOrNull;
      if (otherField == null) {
  ...
  static int _calculateEstimatedSize(List<EventFieldDefinition> fields) =>
      fields.fold(8, (sum, field) => sum + field.typeInfo.estimatedSize);

  static EventComplexity _calculateComplexity(
    List<EventFieldDefinition> fields,
  ) {
    final complexFields = fields.where((f) => f.typeInfo.isComplex).length;
    final nestedFields =
        fields.where((f) => f.typeInfo.hasNestedStructures).length;
  ...
  static EventVersionInfo? _parseVersionInfo(List<String>? docs) {
    ...
    for (final doc in docs) {
      final versionMatch =
          RegExp(r'@version\s+(\d+)\.(\d+)\.?(\d+)?').firstMatch(doc);
...
class EventFieldDefinition {
  ...
  factory EventFieldDefinition.fromIdlField(
    IdlField idlField, {
    Map<String, IdlTypeDef>? customTypes,
  }) {
    final typeInfo =
        EventFieldTypeInfo.fromIdlType(idlField.type, customTypes: customTypes);
  ...
  EventFieldValidationResult validateValue(dynamic value) {
    ...
    if (!typeInfo.isValidValue(value)) {
      errors.add(
        'Invalid type for field $name: expected ${typeInfo.typeName}, got ${value.runtimeType}',
      );
  ...
  static List<EventFieldConstraint> _parseConstraints(List<String>? docs) {
    ...
    for (final doc in docs) {
      ...
      if (minMatch != null) {
        constraints
            .add(EventFieldConstraint.min(int.parse(minMatch.group(1)!)));
      }
      ...
      if (maxMatch != null) {
        constraints
            .add(EventFieldConstraint.max(int.parse(maxMatch.group(1)!)));
      }
      ...
      if (lengthMatch != null) {
        constraints
            .add(EventFieldConstraint.length(int.parse(lengthMatch.group(1)!)));
      }
    }
  ...
  @override
  String toString() =>
      'EventFieldDefinition(name: $name, type: ${typeInfo.typeName})';
}
...
class EventFieldTypeInfo {
  ...
  factory EventFieldTypeInfo.fromIdlType(
    IdlType idlType, {
    Map<String, IdlTypeDef>? customTypes,
  }) =>
      _parseIdlType(idlType, customTypes ?? {});
  ...
  static EventFieldTypeInfo _parseIdlType(
    IdlType idlType,
    Map<String, IdlTypeDef> customTypes,
  ) {
      ...
      case 'option':
        if (idlType.inner != null) {
          ...
          return EventFieldTypeInfo(
            ...
            estimatedSize:
                innerType.estimatedSize + 1, // +1 for option discriminator
      ...
      case 'vec':
        if (idlType.inner != null) {
          ...
          return EventFieldTypeInfo(
            ...
            estimatedSize: 4 +
                (innerType.estimatedSize *
                    4), // 4 bytes for length + estimated content
            typeParameters: [innerType],
          );
        }
        break;
  ...
  static EventFieldTypeInfo _parseCustomType(
    IdlTypeDef typeDef,
    Map<String, IdlTypeDef> customTypes,
  ) {
    ...
    switch (typeName) {
      ...
      default:
        if (typeName.startsWith('Option<')) {
          ...
          final innerTypeName = typeName.substring(
            7,
            typeName.length - 1,
          ); // Remove 'Option<' and '>'
          final innerTypeInfo = EventFieldTypeInfo(
            ...
            isOptional: false,
            hasNestedStructures:
                typeParameters?.first.hasNestedStructures ?? false,
  ...
  bool isCompatibleWith(EventFieldTypeInfo other) {
    ...
    if (numericTypes.contains(typeName) &&
        numericTypes.contains(other.typeName)) {
...
class EventValidationRules {
  ...
  factory EventValidationRules.fromFields(
    ...
    final estimatedSize =
        fields.fold(0, (sum, field) => sum + field.typeInfo.estimatedSize);
    ...
    return EventValidationRules(
      ...
      maxEventSize: config.maxEventSize ??
          (estimatedSize > 1024 ? estimatedSize * 2 : null),
  ...
  EventValidationResult validate(Map<String, dynamic> eventData) {
    ...
    if (maxEventSize != null) {
      ...
      if (jsonStr.length > maxEventSize!) {
        errors.add(
          'Event data size exceeds maximum allowed size: ${jsonStr.length} > $maxEventSize',
        );
      }
    }
...
class EventFieldConstraint {
  ...
  factory EventFieldConstraint.min(num minValue) => EventFieldConstraint(
        type: EventConstraintType.min,
        value: minValue,
      );

  /// Create maximum value constraint
  factory EventFieldConstraint.max(num maxValue) => EventFieldConstraint(
        type: EventConstraintType.max,
        value: maxValue,
      );

  /// Create length constraint
  factory EventFieldConstraint.length(int length) => EventFieldConstraint(
        type: EventConstraintType.length,
        value: length,
      );

  /// Create range constraint
  factory EventFieldConstraint.range(num min, num max) => EventFieldConstraint(
    ...
    switch (type) {
      ...
      case EventConstraintType.length:
        if (fieldValue is String && fieldValue.length != (value as int)) {
          errors.add(
            'String length ${fieldValue.length} does not match required length $value',
          );
        } else if (fieldValue is List && fieldValue.length != (value as int)) {
          errors.add(
            'Array length ${fieldValue.length} does not match required length $value',
          );
        }
        break;
      case EventConstraintType.range:
        ...
        if (fieldValue is num &&
            (fieldValue < range[0] || fieldValue > range[1])) {
          errors.add(
            'Value $fieldValue is not in range [${range[0]}, ${range[1]}]',
          );
        }
        break;
...
class EventDefinitionConfig {
  ...
  factory EventDefinitionConfig.lenient() => const EventDefinitionConfig(
  ...
  factory EventDefinitionConfig.strict() => const EventDefinitionConfig(
        maxEventSize: 2048,
      );
